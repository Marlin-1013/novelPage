...
<body>
	...
	<div class="container">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>				
					<%
						...
						ArrayList<NovelDTO> list = bbsDAO.getList(pageNumber);
						for (int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%= list.get(i).getNum() %></td>
						<td><a href="bbs2.jsp?num=<%= list.get(i).getNum() %>"><%= list.get(i).getTitle()%></a></td>
						<td><%= mdao.checkUser(list.get(i).getUnum()) %></td>
					</tr>
					<%		
						}
					%>
			...
			%>
			<a href="write.jsp" class="btn btn-primary pull-right">소설생성</a>
			<%
				}
			%>
		</div>
